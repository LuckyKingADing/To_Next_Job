#!/usr/bin/env python3
"""
TCMSF初始化评估结果统计脚本 (增强版)

递归读取指定目录下所有的 init_eval_results.csv 文件，跳过 slice* 文件夹，
统计 total_init_time 的各项指标，并分析 reinit_count 情况。

新增功能：
- 支持指定数据源目录，读取 time_ranges.toml 进行场景时间范围统计
- 统计指定场景类型时间范围内 is_success=True 的数据占比
- 输出场景时间范围内未成功的数据详情
"""
...
def parse_time_ranges_toml(filepath: str, type_label: str) -> List[Tuple[float, float]]:
    """
    解析 time_ranges.toml 文件，提取指定 type_label 的时间范围，并转换为绝对时间

    Args:
        filepath: time_ranges.toml 文件路径
        type_label: 要提取的场景类型标签

    Returns:
        时间范围列表，每个元素为 (start, end) 元组（已转换为unix时间戳）
    """
    time_ranges = []
    time_offset = 0.0

    # 读取同目录下的 time_offset.txt
    toml_dir = os.path.dirname(filepath)
    offset_file = os.path.join(toml_dir, 'time_offset.txt')
    if os.path.exists(offset_file):
        try:
            with open(offset_file, 'r', encoding='utf-8') as f:
                time_offset = float(f.read().strip())
    ...
    # 查找目标 type_label 对应的块
    current_label = None

    for i, line in enumerate(lines):
        if '[[time_ranges.type_config]]' in line:
            # 开始新的 type_config 块
            current_label = None
        elif 'type_label' in line and current_label is None:
            # 解析 type_label
            match = re.search(r'type_label\s*=\s*"([^"]+)"', line)
            if match:
                current_label = match.group(1)
        elif 'type_time_range' in line and current_label == type_label:
            # 找到目标标签的时间范围
            # 格式: type_time_range = [[start1, end1], [start2, end2], ...]
            # 使用正则提取所有 [数字, 数字] 对
            ...
            for start_str, end_str in range_matches:
                try:
                    start = float(start_str) + time_offset
                    end = float(end_str)
                    # -1 表示无限制，使用极大值
                    if end == -1:
                        end = float('inf')
...
def find_toml_for_result(result_filepath: str, source_dir: str) -> Optional[str]:
    """
    根据结果文件路径找到对应的 time_ranges.toml 文件

    Args:
        result_filepath: 结果CSV文件路径
        source_dir: 数据源根目录

    Returns:
        time_ranges.toml 文件路径，如果找不到则返回 None
    """
    # 从结果路径中提取数据集标识
    # 例如: /mnt/d/dockers/rt/rtk_pvt/2026/results/0227/pvt_no_camera_2026-02-27_14-38-39/init_eval_results.csv
    # 对应: /mnt/d/dockers/rt/rtk_pvt/2026/0227/time_ranges.toml

    result_path = Path(result_filepath)

    # 获取结果目录中的数据集标识目录名 (如 0227)
    # 假设结构是 results/数据集标识/子数据集/init_eval_results.csv
    ...
    try:
        ...
        if results_idx + 1 < len(parts):
            dataset_id = parts[results_idx + 1]
            toml_path = os.path.join(source_dir, dataset_id, 'time_ranges.toml')
...
def check_record_in_time_range(record: dict, time_ranges: List[Tuple[float, float]]) -> Tuple[bool, bool, float]:
    """
    检查记录是否完全在指定的时间范围内
    需要检查 slice_start_ts、dynamic_ready_ts、init_complete_ts 都在范围内

    Args:
        record: 数据记录
        time_ranges: 时间范围列表（已转换为绝对时间戳）

    Returns:
        (是否在时间范围内, 是否完全在范围内(三个时间点都在), slice_start_ts)
    """
    slice_start_ts = record.get('slice_start_ts', None)
    dynamic_ready_ts = record.get('dynamic_ready_ts', None)
    init_complete_ts = record.get('init_complete_ts', None)

    if slice_start_ts is None:
        return False, False, 0.0

    slice_start_ts = float(slice_start_ts)

    # 检查 slice_start_ts 是否在任意一个时间范围内
    in_range = False
    for start, end in time_ranges:
        if start <= slice_start_ts <= end:
            in_range = True
            break

    if not in_range:
        return False, False, slice_start_ts

    # 检查 dynamic_ready_ts 和 init_complete_ts 是否也在范围内
    # 如果这两个值为 -1（表示无效），则不检查
    fully_in_range = True

    if dynamic_ready_ts is not None and float(dynamic_ready_ts) > 0:
        dynamic_ready_ts = float(dynamic_ready_ts)
        ts_in_range = False
        for start, end in time_ranges:
            if start <= dynamic_ready_ts <= end:
                ts_in_range = True
                break
        if not ts_in_range:
            fully_in_range = False

    if init_complete_ts is not None and float(init_complete_ts) > 0:
        init_complete_ts = float(init_complete_ts)
        ts_in_range = False
        for start, end in time_ranges:
            if start <= init_complete_ts <= end:
                ts_in_range = True
                break
        if not ts_in_range:
            fully_in_range = False

    return True, fully_in_range, slice_start_ts
...
def main():
    parser = argparse.ArgumentParser(
        description='统计TCMSF初始化评估结果 (增强版)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
    # 基本用法
    python init_eval_stats_v2.py /mnt/d/dockers/rt/rtk_pvt/2026/results

    # 指定数据源目录和场景类型
    python init_eval_stats_v2.py /mnt/d/dockers/rt/rtk_pvt/2026/results \
        --source-dir /mnt/d/dockers/rt/rtk_pvt/2026 \
        --type-label 半遮挡

    # 其他选项
    python init_eval_stats_v2.py /mnt/d/dockers/rt/rtk_pvt/2026/results --tolerance 1e-8
        """
    )
    parser.add_argument('directory', help='要搜索的根目录路径（结果目录）')
    parser.add_argument('--source-dir', type=str, default=None,
                        help='数据源目录路径（包含 time_ranges.toml 的目录）')
    parser.add_argument('--type-label', type=str, default=None,
                        help='要统计的场景类型标签，多个场景用逗号分隔（如: 半遮挡,开阔场景,隧道 或单独指定）')
    ...
    if args.source_dir and not os.path.isdir(args.source_dir):
        print(f"错误: 数据源目录 '{args.source_dir}' 不存在")
        sys.exit(1)
    ...
    if args.source_dir:
        output_lines.append(f"数据源目录: {args.source_dir}")
        type_labels = [t.strip() for t in args.type_label.split(',')] if args.type_label else []
        output_lines.append(f"场景类型: {', '.join(type_labels)}")
    ...
    # 场景时间范围相关数据（如果指定了数据源目录）
    # 支持多个场景类型，使用字典存储每个场景的数据
    type_labels = [t.strip() for t in args.type_label.split(',')] if args.type_label else []
    scene_data = {}  # {type_label: {'records': [], 'failed': []}}
    for label in type_labels:
        scene_data[label] = {'records': [], 'failed': []}
    toml_cache = {}  # 缓存已解析的 toml 文件 {(filepath, label): (time_ranges, toml_path)}
    ...
    for filepath in csv_files:
        ...
        for record in filtered_records:
            ...
            # 如果指定了数据源目录和场景类型，检查是否在场景时间范围内
            if args.source_dir and type_labels:
                for label in type_labels:
                    # 查找对应的 time_ranges.toml
                    cache_key = (filepath, label)
                    if cache_key not in toml_cache:
                        toml_path = find_toml_for_result(filepath, args.source_dir)
                        if toml_path:
                            time_ranges = parse_time_ranges_toml(toml_path, label)
                            toml_cache[cache_key] = (time_ranges, toml_path)
                        else:
                            toml_cache[cache_key] = (None, None)

                    time_ranges, toml_path = toml_cache[cache_key]

                    if time_ranges:
                        in_range, fully_in_range, _ = check_record_in_time_range(record, time_ranges)
                        if in_range:
                            # 记录在场景时间范围内
                            record['_toml_path'] = toml_path
                            scene_data[label]['records'].append(record)

                            # 检查是否成功：需要 is_success=True 且完全在时间范围内
                            # 如果 init_complete_ts 在时间范围外，即使 is_success=True 也算失败
                            if not record.get('is_success', False) or not fully_in_range:
                                scene_data[label]['failed'].append(record)
    ...
    if args.source_dir and type_labels:
        for label in type_labels:
            records_in_scene_range = scene_data[label]['records']
            records_in_scene_failed = scene_data[label]['failed']

            if records_in_scene_range:
                output_lines.append("")
                output_lines.append("=" * 60)
                output_lines.append(f"场景 '{label}' 时间范围统计")
                output_lines.append("=" * 60)

                scene_total = len(records_in_scene_range)
                # 成功记录数 = 总数 - 失败数（失败包括is_success=False或时间点超出范围）
                scene_success_count = scene_total - len(records_in_scene_failed)

                output_lines.append(f"  场景时间范围内记录总数: {scene_total}")
                output_lines.append(f"  成功记录数 (完全在时间范围内且is_success=True): {scene_success_count}")

                if scene_total > 0:
                    success_percentage = (scene_success_count / scene_total) * 100
                    output_lines.append(f"  成功占比: {success_percentage:.2f}%")

                # 输出场景时间范围内未成功的记录详情
                if records_in_scene_failed:
                    output_lines.append(f"  未成功记录详情 ({len(records_in_scene_failed)} 条):")
                    # 按文件分组输出
                    files_with_failed = {}
                    for record in records_in_scene_failed:
                        filepath = record.get('_filepath', 'unknown')
                        if filepath not in files_with_failed:
                            files_with_failed[filepath] = []
                        files_with_failed[filepath].append(record)

                    for filepath, records in files_with_failed.items():
                        output_lines.append(f"    文件: {filepath}")
                        toml_path = records[0].get('_toml_path', 'unknown')
                        output_lines.append(f"    time_ranges.toml: {toml_path}")
                        for record in records:
                            output_lines.append(f"      {format_record_line(record)}")
