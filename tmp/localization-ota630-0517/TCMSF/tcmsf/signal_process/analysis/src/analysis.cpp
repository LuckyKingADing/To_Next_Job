...
VDSA::RESULT VDSA::operator()(const Eigen::Vector3d &vel_, const Eigen::Vector3d &gyro_, double dt, double acc_lon_sm, double acc_lat_sm) {

    VDSA::RESULT result;
    auto         EPS = std::numeric_limits<double>::epsilon();

    vel_queue.push_back(vel_.norm());
    while (vel_queue.size() > BUFFER_SIZE) {
        vel_queue.pop_front();
    }

    {
        ...
        latitudinal_acc = -vn * wz;
        result.lat_ac   = 1;
    }

    {
        if (vel_queue.size() == BUFFER_SIZE) {
            size_t idx_pre_half = BUFFER_SIZE / 2;

            longitudinal_acc = (vel_queue.back() - vel_queue[idx_pre_half]) / (std::abs(dt) * ((float)BUFFER_SIZE / 2) + EPS);
            result.lon_ac    = 1;
        }
    }

    {
        ...
        double f_lat = std::abs(latitudinal_acc);
        double f_lon = std::abs(longitudinal_acc);
...

        {
            ...
            if (std::abs(acc_lon_sm) > 0.15 || std::abs(acc_lat_sm) > 0.15) {
                ...
                acc_factor = 20 * (std::abs(acc_lon_sm) > std::abs(acc_lat_sm) ? std::abs(acc_lon_sm) : std::abs(acc_lat_sm));
            } else {
                acc_factor = 1.0;
            }
...

        {
            if (acc_lonlat_count % ACC_LONLAT_SKIP == 0) {
                dacc_lon_sm    = (acc_lon_sm - acc_lon_sm_pre) / (std::abs(dt) * ACC_LONLAT_SKIP + EPS);
                dacc_lat_sm    = (acc_lat_sm - acc_lat_sm_pre) / (std::abs(dt) * ACC_LONLAT_SKIP + EPS);
            ...
            if (std::abs(dacc_lon_sm) > 0.15 || std::abs(dacc_lat_sm) > 0.15) {
                ...
                dacc_factor = 20 * (std::abs(dacc_lon_sm) > std::abs(dacc_lat_sm) ? std::abs(dacc_lon_sm) : std::abs(dacc_lat_sm));
            } else {
                dacc_factor = 1.0;
            }
            if (std::abs(dacc_factor) < 1) {
                dacc_factor = 1.0;
            }
...
// void VDSA::statics() {
//     max_ = -Eigen::Matrix<double, 9, 1>::Ones() * std::numeric_limits<double>::max();
//     min_ = Eigen::Matrix<double, 9, 1>::Ones() * std::numeric_limits<double>::max();
...
// bool VDSA::is_static() {
//     statics();
//     bool is_zero_speed      = (max_.block(3, 1, 0, 0).array().abs() > cfg.static_bound.block(3, 1, 0, 0).array()).sum() == 0;
//     bool is_imu_static      = ((max_ - min_).array().abs() > cfg.static_bound.array()).sum() == 0;
//     bool is_gyro_mean_valid = ((gyro_mean_left_ - gyro_mean_middle_).array().abs() > cfg.gyro_bias_valid_mean_bound.array()).sum() == 0 &&
//                               ((gyro_mean_right_ - gyro_mean_middle_).array().abs() > cfg.gyro_bias_valid_mean_bound.array()).sum() == 0;
//     return is_zero_speed && is_imu_static && is_gyro_mean_valid;
// }

// VDSA::RESULT VDSA::operator()(const Eigen::Vector3d &vel_, const Eigen::Vector3d &acc_, const Eigen::Vector3d &gyro_, double dt, double acc_lon_sm, double acc_lat_sm) {
//     count++;
//     VDSA::RESULT result;
//     auto         EPS = std::numeric_limits<double>::epsilon();
...
//         latitudinal_acc = -vn * wz; /// earth.g.norm();
...
//             longitudinal_acc = (vel[idx].y() - vel[idx_pre_half].y()) / (std::abs(dt) * ((float)BUFFER_SIZE / 2) + EPS); // /                               earth.g.norm();
...
//         double gyro_bias_bound = 0.08 / 180.0 * M_PI;
//         gyro_bias.x()          = gyro_bias.x() > gyro_bias_bound ? gyro_bias_bound : gyro_bias.x();
//         gyro_bias.x()          = gyro_bias.x() < -gyro_bias_bound ? -gyro_bias_bound : gyro_bias.x();
//         gyro_bias.y()          = gyro_bias.y() > gyro_bias_bound ? gyro_bias_bound : gyro_bias.y();
//         gyro_bias.y()          = gyro_bias.y() < -gyro_bias_bound ? -gyro_bias_bound : gyro_bias.y();
//         gyro_bias.z()          = gyro_bias.z() > gyro_bias_bound ? gyro_bias_bound : gyro_bias.z();
//         gyro_bias.z()          = gyro_bias.z() < -gyro_bias_bound ? -gyro_bias_bound : gyro_bias.z();
//         result.zupt            = 1;
//     }
...
// // DA::RESULT DA::operator()(const DrData &dr_data, const MsfData &msf_data, double whl_spd, double dt, double rtk_fix_time) {
// //     DA::RESULT result;
...
// //     v_ref[idx] = sqrt(msf_data.vel_x * msf_data.vel_x + msf_data.vel_y * msf_data.vel_y + msf_data.vel_z * msf_data.vel_z);
...
// //         if (whl_spd_factor > 1.005) {
// //             whl_spd_factor = 1.005;
// //         }
// //         if (whl_spd_factor < 0.995) {
// //             whl_spd_factor = 0.995;
// //         }
// //     } else {
// //         result.spd_sa = 0;
// //     }

// //     if (rtk_fix_time > 20.0 && std::abs(min_spd_whl) > 3.0) {
// //         gyro_bias << msf_data.bias_gyro_x, msf_data.bias_gyro_y, msf_data.bias_gyro_z;
