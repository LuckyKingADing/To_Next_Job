...
/// @brief Factor to restrict the degrees of freedom of optimization (e.g., fixing roll, pitch rotation).
/// @note  This factor only enables *soft* constraints. The source point cloud can move to the restricted directions slightly).
struct DoFDamperFactor {
  /// @brief Constructor.
  DoFDamperFactor() {
    damper.setZero();
  }

  /// @brief Set rotation damper. 
  void set_rotation_mask(const Eigen::Array3d& rot_mask) { damper.head<3>() = rot_mask; }

  /// @brief  Set translation damper. 
  void set_translation_mask(const Eigen::Array3d& trans_mask) { damper.tail<3>() = trans_mask; }
  ...
  void update_linearized_system(
    ...
    Eigen::Matrix<double, 6, 1>* b,
    double* e) const {
    *H +=  (damper + 1.0).abs().matrix().asDiagonal();
  }
  ...
  Eigen::Array<double, 6, 1> damper;  ///< damper for restricting DoF (rx, ry, rz, tx, ty, tz)
};
