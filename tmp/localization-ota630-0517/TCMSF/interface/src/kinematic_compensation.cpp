...
// 保证数据帧在时间上是有序的
void KinematicCompensation::insert(const MSF::KinematicData &motion_data) {
    ...
    if (!motion_queue.empty() &&
        ...
        // }
        AWARN << "[time disorder detected] KinematicData";
        return;
    }
...
std::pair<int, MSF::KinematicData> KinematicCompensation::pin(double t) {
    int                rslt_ = 0x0;
    MSF::KinematicData kstate;

    std::unique_lock<std::recursive_mutex> lock(motion_queue_lock);
    if (motion_queue.empty()) {
        AWARN << "KinematicCompensation, empty queue";
        rslt_ |= 0x1; // empty queue
        return std::make_pair(rslt_, kstate);
    }

    // 插值，依据两帧KinematicData数据和时间戳，进行前拓、插值、后拓
    auto interpolation_func = [&rslt_, this](const MSF::KinematicData &data1, const MSF::KinematicData &data2, double t) -> MSF::KinematicData {
        MSF::KinematicData interpolation_data;
        ...
        if (dt_left < -MIN_SAMPLING_GAP * 3) {
            AINFO << "dt_left: " << std::setprecision(6) << dt_left;
            rslt_ |= 0x2; // [left] out of range
        }

        // 后拓超过3倍最小采样间隔，认为无法保证精度
        if (dt_right > MIN_SAMPLING_GAP * 3) {
            AINFO << "dt_right: " << std::setprecision(6) << dt_right;
    ...
    // 在队列里面找到时间戳的位置，并插值
    auto pin_func = [this, interpolation_func, &rslt_](double t) -> MSF::KinematicData {
        // KinematicData的顺序关系由时间戳确定
        auto t_cmp_func = [](const MSF::KinematicData &data, double t) -> bool { return data.measurement_timestamp < t; };
...
std::pair<int, MSF::KinematicData> KinematicCompensation::delta(double t1, double t2, bool use_veh_frame = false) {

    int                rslt_ = 0x0;
    MSF::KinematicData delta_1to2;

    auto r1 = pin(t1);
    auto r2 = pin(t2);
    rslt_   = r1.first | r2.first;

    if (rslt_ & 0x1) {
        return std::make_pair(rslt_, delta_1to2);
    }

    delta_1to2 = r2.second - r1.second;

    // 返回补偿状态码和结果
    if (use_veh_frame) {
        // 自车系
        Eigen::Matrix3d    mat_p1         = r1.second.att.toRotationMatrix().transpose();
        MSF::KinematicData delta_1to2_veh = delta_1to2;
